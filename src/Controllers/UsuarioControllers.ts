...
import { createUsers, GetUsers, GetUsersId, VerificarEmail } from '../models/usuarioModel.js'
import { hashpassword } from './AdvogadoControllers.js'
import axios from 'axios'
import { v4 as uuidv4 } from 'uuid';
const router = express.Router()

router.get("/getuser", async (_, res: Response)=>{
...
router.get("/getuser/:id", async(req: Request, res: Response)=>{
    const {id} = req.params
    const dados = await GetUsersId(id)
    return res.status(200).json({ dados })
})
